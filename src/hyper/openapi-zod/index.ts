...
function toJson(schema: ZodLike): JsonSchema {
  ...
  switch (name) {
    ...
    case "ZodArray":
    case "array": {
      // Local edit (source-distributed component): cast via `unknown` because
      // ZodDef.type is typed `string` but Zod's array def carries the element
      // schema under `type` (v3) / `element` (v4).
      const v = def as unknown as { type?: ZodLike; element?: ZodLike }
